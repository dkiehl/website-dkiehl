...
<%
	...
	urlfilter=request.querystring("filter")
	if urlfilter <> "" then
		getfilter=urlfilter
	end if
	...
	getuser=request.form("myuser")
	urluser=request.querystring("user")
	if urluser <> "" then
	   getuser=urluser
	end if
%>
...
<form method="post" action="/index.asp">
  ...
  <input type="hidden" id="myuser" name="myuser">
...
<%
	...
	If GetFilter2 = "" then
		sql = "select * from [PageTable] where [user] in ('','" + inUser + "') order by [Sequence]"
	else
		sql = "select * from [PageTable] where [user] in ('','" + inUser + "') order by [Sequence]"
		sql = "select * from [PageTable] where [user] in ('','" + inUser + "') and [LineType] in ('A','B') union "
		sql = sql & "select * from [PageTable] where [user] in ('','" + inUser + "') and [LineText] like '" + GetFilter2 + "' order by [Sequence]"		
	end if

	If GetFilter = "DAK" then
		sql = "select * from [PageTable] order by [Sequence]"
        end if
	...
	do while not RS.EOF
	  ...
	  select case LineType
		...
		Case "B":
			...
			LineB = LineB & "<div style=""float: right; text-align: right; width=50%;"">" & rightstr & "</div>"
		...
		Case "D":
			...
			LineC = LineC & "<div style=""float: left; text-align: left; width=50%;"">" & leftstr & ptr & "</div>"
			LineC = LineC & "<div style=""float: right; text-align: right; width=50%;"">" & rightstr & " (Live Data)</div>"
			LineC = LineC & "</button></center></td></tr>"
